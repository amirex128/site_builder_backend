...
func Register(router *gin.Engine, pgClient *postgres.Postgres, redisClient *redis.Redis, esClient *elasticsearch.Elasticsearch) {
	// User routes
	userGroup := router.Group("User")
	user_router.UserRegister(userGroup, pgClient.DB)

	addressGroup := router.Group("Address")
	user_router.AddressRegister(addressGroup, pgClient.DB)
}
